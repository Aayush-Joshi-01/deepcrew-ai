...
def main() -> None:
    ...
    parser.add_argument(
        "--version", action="version", version=f"deepcrew-ai {__version__}"
    )
    ...
    run_p.add_argument(
        "--no-stream", action="store_true", help="Print final output only"
    )
    ...
    list_p.add_argument(
        "--config", "-c", required=True, help="Path to workflow YAML"
    )
